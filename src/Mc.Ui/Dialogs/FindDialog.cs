...
public sealed class FindOptions
{
    ...
    public bool SearchInArchives { get; set; }  // NEW: search inside archive files
...
public static class FindDialog
{
    public static FindOptions? Show(string startDir)
    {
        ...
        var archivesCb    = new CheckBox { X = 1,  Y = 13, Text = "Search in archives",       CheckedState = CheckState.UnChecked, ColorScheme = McTheme.Dialog };
        var caseCb        = new CheckBox { X = 1,  Y = 14, Text = "Case sensitive",            CheckedState = CheckState.UnChecked, ColorScheme = McTheme.Dialog };
        var regexCb       = new CheckBox { X = 35, Y = 14, Text = "Use regex",                 CheckedState = CheckState.UnChecked, ColorScheme = McTheme.Dialog };
        d.Add(subDirsCb, followSymCb, skipHiddenCb, archivesCb, caseCb, regexCb);

        // ── Ignore dirs (#33) ─────────────────────────────────────────
        d.Add(new Label { X = 1, Y = 16, Text = "Ignore dirs (colon-separated):" });
        var ignoreDirsInput = new TextField { X = 1, Y = 17, Width = 66, Height = 1, Text = string.Empty, ColorScheme = McTheme.Dialog };
        d.Add(ignoreDirsInput);

        // ── Date filters (#7) ─────────────────────────────────────────
        d.Add(new Label { X = 1, Y = 19, Text = "Newer than (days, 0=any):" });
        var newerThanInput = new TextField { X = 26, Y = 19, Width = 6, Text = "0", ColorScheme = McTheme.Dialog };
        d.Add(new Label { X = 34, Y = 19, Text = "Older than (days, 0=any):" });
        var olderThanInput = new TextField { X = 59, Y = 19, Width = 6, Text = "0", ColorScheme = McTheme.Dialog };
        d.Add(newerThanInput, olderThanInput);

        // ── Size filters (#8) ─────────────────────────────────────────
        d.Add(new Label { X = 1, Y = 21, Text = "Min size KB (0=any):" });
        var minSizeInput = new TextField { X = 21, Y = 21, Width = 10, Text = "0", ColorScheme = McTheme.Dialog };
        d.Add(new Label { X = 34, Y = 21, Text = "Max size KB (0=any):" });
        var maxSizeInput = new TextField { X = 55, Y = 21, Width = 10, Text = "0", ColorScheme = McTheme.Dialog };
        d.Add(minSizeInput, maxSizeInput);

        d.Height = 25; // expand dialog to fit new rows
        ...
        ok.Accepting += (_, _) =>
        {
            ...
            result = new FindOptions
            {
                ...
                SearchInArchives = archivesCb.CheckedState  == CheckState.Checked,
